...
A plain drawer that will render HTML or oui-drawer elements. Closable by clicking on the close button, the backdrop or by setting the prop opened to `false`. It also emits 'close' and 'open' events that can be used to react to user interaction. By default the size is set to auto and will expand to fit the content width, but it can be overwritten by passing the desired width to the size prop with the unit (e.g. size="20em").
...
> Position prop set to right

![oui-drawer position right](screenshot01.png)
...
When used with `oui-collapsable` and `oui-drawer-item` the children will inherit some styling. e.g. will swap alignment on when position is set to right.
