    ...
    var Select = React.createClass({
        ...
        buildMenu: function() {
            ...
            var ops = Object.keys(this.state.filteredOptions).map(function(key) {
                ...
                var out;
                if (op.disabled) {
                    out = <div ref={ref} key={'option-' + op.value} className={optionClass}>{op.label}</div>;
                } else {
                    out = <div ref={ref} key={'option-' + op.value} className={optionClass} onMouseEnter={mouseEnter} onMouseLeave={mouseLeave} onMouseDown={mouseDown} onClick={mouseDown}>{ op.create ? 'Add ' + op.label + ' ?' : op.label}</div>;
                }
                return out;
            }, this);
